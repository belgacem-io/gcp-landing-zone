...
module "templates" {
  ...
  metadata = {
    user-data = templatefile("${path.module}/assets/gw.yaml", { commands = [
    ...
    "iptables -A FORWARD -s ${join(",", flatten(values(var.regional_aggregates)))} -d ${join(",", flatten(values(var.regional_aggregates)))} -j ACCEPT",
      # Drop everything else
